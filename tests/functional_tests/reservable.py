...
class TestbedWithReservationTestCase(ManifoldTestCase):
    ...
    def test_list_leases(self):
        ...
        """
        Q = 'SELECT resource, start_time, end_time FROM %slease' % (self.NAMESPACE, )
        records = self._run(Q)

        # We might count the leases, add one, and check that we have one more...

    def test_list_slice_resources(self):
        Q = 'SELECT hrn FROM %sresource WHERE slice == "%s"' % (self.NAMESPACE, self.SLICE_URN, )
        records = self._run(Q)
        

    def test_list_slice_leases(self):
        Q = 'SELECT resource, start_time, end_time FROM %slease WHERE slice == "%s"' % (self.NAMESPACE, self.SLICE_URN, )
        records = self._run(Q)
    ...
    def test_add_lease(self):
        ...
        """
        Q = 'UPDATE %sslice SET resource = ["%s"], lease = [{resource: "%s", start_time: "%s", end_time: "%s"}] where slice_hrn == "%s"' % (self.NAMESPACE, self.NODE_1, self.NODE_1, self.START, self.END, self.SLICE_HRN, )
        records = self._run(Q)
    ...
    def test_add_two_leases(self):
        ...
        """
        Q = 'UPDATE %sslice SET resource = ["%s"], lease = [{resource: "%s", start_time: "%s", end_time: "%s"}, {resource: "%s", start_time: "%s", end_time: "%s"}] where slice_hrn == "%s"' % (self.NAMESPACE, self.NODE_1, self.NODE_1, self.START, self.END, self.NODE_1, self.START+3600, self.END+3600, self.SLICE_HRN, )
        records = self._run(Q)
    ...
    def test_add_two_leases_2(self):
        ...
        """
        Q = 'UPDATE %sslice SET resource = ["%s"], lease = [{resource: "%s", start_time: "%s", end_time: "%s"}, {resource: "%s", start_time: "%s", end_time: "%s"}] where slice_hrn == "%s"' % (self.NAMESPACE, self.NODE_1, self.NODE_1, self.START, self.END, self.NODE_2, self.START+3600, self.END+3600, self.SLICE_HRN, )
        records = self._run(Q)
    ...
    def test_add_two_leases_3(self):
        ...
        """
        Q = 'UPDATE %sslice SET resource = ["%s", "%s"], lease = [{resource: "%s", start_time: "%s", end_time: "%s"}, {resource: "%s", start_time: "%s", end_time: "%s"}] where slice_hrn == "%s"' % (self.NAMESPACE, self.NODE_1, self.NODE_2, self.NODE_1, self.START, self.END, self.NODE_2, self.START+3600, self.END+3600, self.SLICE_HRN, )
        records = self._run(Q)

    def test_clear_slice(self):
        Q = 'UPDATE %sslice SET resource = [], lease = [] where slice_hrn == "%s"' % (self.NAMESPACE, self.SLICE_HRN, )
        records = self._run(Q)
...
class NITOSTests(TestbedWithReservationTestCase):
    ...
    NODE_1 = 'urn:publicid:IDN+omf:nitos+node+node047'
    #NODE_2 = 'urn:publicid:IDN+omf:nitos+node+omf.nitos.node036'
    NODE_2 = 'urn:publicid:IDN+omf:nitos+node+node041'
