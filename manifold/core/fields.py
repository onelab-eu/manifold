...
from manifold.core.field            import Field, merge_field 
from manifold.util.log              import Log # DEBUG
...
class Fields(list):
    ...
    def __and__(self, fields):
        ...
        """
        assert isinstance(fields, Fields), "Invalid fields = %s (%s)" % (fields, type(fields))
        ...
        #   return Fields([x for x in self if x in fields])
            ...
            for field1 in self:
                for field2 in fields:
                    # merge_field may trigger an exception in case of exception.
                    field = merge_field(field1, field2)
                    if field:
                        ret.add(field)
                        break
        return ret 
...
#MANDO            self[:] = [x for x in self if x in fields]
            self = (self & fields)
        return self
    ...
    def __eq__(self, other):
        ...
        """
        assert isinstance(other, Fields), "Invalid other = %s (%s)" % (other, type(other))
